...
| 维度 | 当前规模 | 说明 |
| --- | ---: | --- |
| 知识条目 | 38 | 经过结构化整理的公开内容 |
| 内容来源 | 4 类 | 微信公众号 21、网页 14、X 1、小红书 2 |
| 主题 | 84 | 用于跨文章聚合知识 |
| 人物 | 69 | 连接作者、研究者与观点来源 |
| 概念 | 363 | 构成可检索的认知坐标 |
| 产品经理专题 | 5 | 前端基础 1 篇；后端相关 4 篇（Nginx、Gateway、数据库、Redis） |
...
└── 产品经理 Wiki
    ...
    ├── 技术通识
    │   ├── 前端：页面、交互、状态与性能
    │   └── 后端：Nginx、Gateway、数据库、Redis
    ├── 市场与商业：市场判断、竞品、增长与商业模型
    └── 实践与思考：工作现场的判断、复盘与方法沉淀
```

产品经理 Wiki 当前先完成前后端技术通识底图，帮助产品经理从一次用户请求出发，理解请求如何依次经过 **浏览器 / 前端 → Nginx → Gateway → 业务服务 → 数据库 / Redis**，以及每一层会影响哪些产品体验、系统风险和需求决策。
...
- 支持专题搜索和分类筛选。
- 用请求链路图解释系统组件之间的关系。
- 技术通识可按前端 / 后端分区阅读；当前包含前端基础，以及 Nginx、Gateway、数据库、Redis 四篇后端相关知识。
